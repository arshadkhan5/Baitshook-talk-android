...
package com.gcc.talk.gccExtensions
...
import com.gcc.talk.gccUtils.GccTextDrawable
...
@Deprecated("use other constructor that expects com.nextcloud.talk.models.domain.GccConversationModel")
...
fun ImageView.loadConversationAvatar(
    ...
    val imageRequestUri = GccApiUtils.getUrlForConversationAvatarWithVersion(
        1,
        user.baseUrl,
        conversation.token,
        GccDisplayUtils.isDarkModeOn(this.context),
...
fun ImageView.loadUserAvatar(
    ...
    val imageRequestUri = GccApiUtils.getUrlForAvatar(
        user.baseUrl!!,
        avatarId,
        requestBigSize
    )
...
fun ImageView.loadFederatedUserAvatar(message: GccChatMessage): io.reactivex.disposables.Disposable {
    val cloudId = message.actorId!!
    val darkTheme = if (GccDisplayUtils.isDarkModeOn(context)) 1 else 0
...
fun ImageView.loadFederatedUserAvatar(
    ...
    val imageRequestUri = GccApiUtils.getUrlForFederatedAvatar(
        baseUrl,
        token,
        cloudId,
        darkTheme,
    ...
    return DisposableWrapper(
        load(url) {
            user?.let {
                addHeader(
                    "Authorization",
                    GccApiUtils.getCredentials(user.username, user.token)!!
                )
            }
...
fun ImageView.loadAvatarWithUrl(user: GccUser? = null, url: String): io.reactivex.disposables.Disposable =
...
fun ImageView.loadThumbnail(url: String, user: GccUser): io.reactivex.disposables.Disposable {
    ...
    layers[1] = ContextCompat.getDrawable(context, R.drawable.gcc_icon)
...
fun ImageView.loadImage(url: String, user: GccUser, placeholder: Drawable? = null): io.reactivex.disposables.Disposable {
...
fun ImageView.loadBotsAvatar(): io.reactivex.disposables.Disposable {
    ...
    layers[1] = GccTextDrawable(context, ">")
    val layerDrawable = LayerDrawable(layers)
    val data: Any = layerDrawable
...
fun ImageView.loadGuestAvatar(user: GccUser, name: String, big: Boolean): io.reactivex.disposables.Disposable =
...
fun ImageView.loadGuestAvatar(baseUrl: String, name: String, big: Boolean): io.reactivex.disposables.Disposable {
    val imageRequestUri = GccApiUtils.getUrlForGuestAvatar(
        baseUrl,
        name,
        big
    )
