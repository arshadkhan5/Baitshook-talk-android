...
import com.gcc.talk.gccJobs.GccSaveFileToStorageWorker
...
class SaveToStorageDialogFragment : DialogFragment() {
    ...
    private fun saveImageToStorage(fileName: String) {
        ...
        val data: Data = Data.Builder()
            .putString(GccSaveFileToStorageWorker.KEY_FILE_NAME, fileName)
            .putString(GccSaveFileToStorageWorker.KEY_SOURCE_FILE_PATH, "$sourceFilePath/$fileName")
            .build()

        val saveWorker: OneTimeWorkRequest = OneTimeWorkRequest.Builder(GccSaveFileToStorageWorker::class.java)
            .setInputData(data)
            .addTag(workerTag)
            .build()
