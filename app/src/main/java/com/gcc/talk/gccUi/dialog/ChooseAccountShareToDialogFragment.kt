...
import com.gcc.talk.databinding.DialogChooseAccountShareToBinding
...
class ChooseAccountShareToDialogFragment : DialogFragment() {
    ...
    private var adapter: FlexibleAdapter<GccAdvancedUserItem>? = null
    private val userItems: MutableList<GccAdvancedUserItem> = ArrayList()
    ...
    private fun setupCurrentUser(user: GccUser?) {
        ...
        if (adapter == null) {
            ...
            for (userItem in userManager!!.users.blockingGet()) {
                ...
                if (!userEntity.current) {
                    ...
                    userItems.add(GccAdvancedUserItem(participant, userEntity, null, viewThemeUtils, 0))
                }
            }
    ...
    private fun setupListeners(user: GccUser) {
