...
class GccContactAddressBookWorker(val context: Context, workerParameters: WorkerParameters) :
    ...
    override fun doWork(): Result {
        ...
        if (!force) {
            if (System.currentTimeMillis() - appPreferences.getPhoneBookIntegrationLastRun(0L) <
                GccDateConstants.DAYS_DIVIDER *
                GccDateConstants.HOURS_DIVIDER *
                GccDateConstants.MINUTES_DIVIDER *
        ...
        if (deviceContactsWithNumbers.isNotEmpty()) {
            ...
            ncApi.searchContactsByPhoneNumber(
                ...
                GccApiUtils.getUrlForSearchByNumber(currentUser.baseUrl!!),
        ...
        fun createLinkedAccount(lookupKey: String, cloudId: String) {
            ...
            if (contactCursor != null) {
                if (contactCursor.count > 0) {
                    ...
                    val displayName = GccContactUtils.getDisplayNameFromDeviceContact(context, id)
        ...
        fun run(context: Context) {
            if (ContextCompat.checkSelfPermission(context, Manifest.permission.WRITE_CONTACTS) ==
                ...
                WorkManager.getInstance().enqueue(
                    OneTimeWorkRequest.Builder(GccContactAddressBookWorker::class.java)
