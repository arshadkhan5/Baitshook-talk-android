...
import com.gcc.talk.databinding.ItemSystemMessageBinding
...
class GccSystemMessageViewHolder(itemView: View) :
    MessageHolders
        .IncomingTextMessageViewHolder<GccChatMessage>(itemView) {
    ...
    @JvmField
    @Inject
    var appPreferences: GccAppPreferences? = null
    ...
    @JvmField
    @Inject
    var dateUtils: GccDateUtils? = null
    protected var background: ViewGroup

    lateinit var systemMessageInterface: GccSystemMessageInterface
    ...
    override fun onBind(message: GccChatMessage) {
        ...
        val bubbleDrawable = GccDisplayUtils.getMessageSelector(
        ...
        if (message.messageParameters != null && message.messageParameters!!.size > 0) {
            for (key in message.messageParameters!!.keys) {
                ...
                if (individualMap != null && individualMap.containsKey("name")) {
                    ...
                    messageString =
                        GccDisplayUtils.searchAndColor(
                            messageString,
                            searchText!!,
                            mentionColor,
    ...
    @SuppressLint("SetTextI18n")
    private fun processExpandableParent(message: GccChatMessage, messageString: Spannable) {
    ...
    fun assignSystemMessageInterface(systemMessageInterface: GccSystemMessageInterface) {
