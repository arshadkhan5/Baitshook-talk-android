...
import com.gcc.talk.databinding.ReferenceInsideMessageBinding
...
class GccLinkPreview {

    fun showLink(message: GccChatMessage, ncApi: GccNcApi, binding: ReferenceInsideMessageBinding, context: Context) {
        ...
        if (!message.extractedUrlToPreview.isNullOrEmpty()) {
            val credentials: String = GccApiUtils.getCredentials(message.activeUser?.username, message.activeUser?.token)!!
            val openGraphLink = GccApiUtils.getUrlForOpenGraph(message.activeUser?.baseUrl!!)
            ncApi.getOpenGraph(
                credentials,
                openGraphLink,
    ...
    companion object {
        private val TAG = GccLinkPreview::class.java.simpleName
