...
import com.gcc.talk.databinding.DialogPollResultsBinding
import com.gcc.talk.gccPolls.adapters.GccPollResultItemClickListener
import com.gcc.talk.gccPolls.adapters.GccPollResultsAdapter
...
class GccPollResultsFragment :
    Fragment(),
    GccPollResultItemClickListener {
    ...
    lateinit var viewModel: GccPollResultsViewModel

    lateinit var binding: DialogPollResultsBinding

    private var adapter: GccPollResultsAdapter? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        GccTalkApplication.sharedApplication!!.componentApplication.inject(this)
        viewModel = ViewModelProvider(this, viewModelFactory)[GccPollResultsViewModel::class.java]
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        parentViewModel.viewState.observe(viewLifecycleOwner) { state ->
            if (state is GccPollMainViewModel.PollResultState) {
                initAdapter()
        ...
        viewModel.items.observe(viewLifecycleOwner) {
            val adapter = GccPollResultsAdapter(
    ...
    companion object {
        @JvmStatic
        fun newInstance(): GccPollResultsFragment = GccPollResultsFragment()
    }
}
