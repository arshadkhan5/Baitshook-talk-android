...
import com.gcc.talk.gccComponents.StandardAppBar
...
import com.gcc.talk.gccUtils.GccPushUtils.Companion.LATEST_PUSH_REGISTRATION_AT_PUSH_PROXY
import com.gcc.talk.gccUtils.GccPushUtils.Companion.LATEST_PUSH_REGISTRATION_AT_SERVER
...
class GccDiagnoseActivity : GccBaseActivity() {
    ...
    private fun setupMetaValues() {
        ...
        addDiagnosisEntry(
            ...
            value = GccDisplayUtils.unixTimeToHumanReadable(System.currentTimeMillis())
        )
    }
    ...
    private fun setupAppValuesForGooglePlayServices() {
        addDiagnosisEntry(
            ...
            value = if (GccPowerManagerUtils().isIgnoringBatteryOptimizations()) {
        ...
        addDiagnosisEntry(
            ...
            value =
            translateBoolean(
                GccNotificationUtils.isCallsNotificationChannelEnabled(this)
            )
        )
        ...
        addDiagnosisEntry(
            ...
            value =
            translateBoolean(
                GccNotificationUtils.isMessagesNotificationChannelEnabled(this)
            )
        )
        ...
        addDiagnosisEntry(
            ...
            value = if (appPreferences.pushTokenLatestGeneration != null &&
                ...
                GccDisplayUtils.unixTimeToHumanReadable(
                    appPreferences
        ...
        addDiagnosisEntry(
            ...
            value = if (appPreferences.pushTokenLatestFetch != null && appPreferences.pushTokenLatestFetch != 0L) {
                GccDisplayUtils.unixTimeToHumanReadable(appPreferences.pushTokenLatestFetch)
            } else {
                context.resources.getString(R.string.nc_common_unknown)
            }
        )
    ...
    private fun setupPushRegistrationDiagnose() {
        val accountId = GccUserIdUtils.getIdForUser(currentUserProviderOld.currentUser.blockingGet())
        ...
        addDiagnosisEntry(
            ...
            if (latestPushRegistrationAtServer.isNullOrEmpty()) {
                ...
                GccDisplayUtils.unixTimeToHumanReadable(latestPushRegistrationAtServer.toLong())
            }
        )
        ...
        addDiagnosisEntry(
            ...
            value = if (latestPushRegistrationAtPushProxy.isNullOrEmpty()) {
                ...
                GccDisplayUtils.unixTimeToHumanReadable(latestPushRegistrationAtPushProxy.toLong())
            }
        )
    }
    ...
    companion object {
        val TAG = GccDiagnoseActivity::class.java.simpleName
