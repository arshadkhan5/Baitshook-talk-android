...
import com.gcc.talk.databinding.ActivityMessageSearchBinding
...
class GccMessageSearchActivity : GccBaseActivity() {
    ...
    private lateinit var user: GccUser

    private lateinit var viewModel: GccMessageSearchViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this, viewModelFactory)[GccMessageSearchViewModel::class.java]
        user = currentUserProviderOld.currentUser.blockingGet()
        val roomToken = intent.getStringExtra(GccBundleKeys.KEY_ROOM_TOKEN)!!
    ...
    private fun setupActionBar() {
        ...
        val conversationName = intent.getStringExtra(GccBundleKeys.KEY_CONVERSATION_NAME)
    ...
    private fun setupStateObserver() {
        viewModel.state.observe(this) { state ->
            when (state) {
                GccMessageSearchViewModel.InitialState -> showInitial()
                GccMessageSearchViewModel.EmptyState -> showEmpty()
                is GccMessageSearchViewModel.LoadedState -> showLoaded(state)
                GccMessageSearchViewModel.LoadingState -> showLoading()
                GccMessageSearchViewModel.ErrorState -> showError()
                is GccMessageSearchViewModel.FinishedState -> onFinish()
            }
        }
    }
    ...
    private fun showLoaded(state: GccMessageSearchViewModel.LoadedState) {
        displayLoading(false)
    ...
    private fun setAdapterItems(state: GccMessageSearchViewModel.LoadedState) {
        val loadMoreItems = if (state.hasMore) {
            listOf(GccLoadMoreResultsItem)
        } else {
            emptyList()
        }
        val newItems =
            state.results.map { GccMessageResultItem(this, user, it, false, viewThemeUtils) } + loadMoreItems
    ...
    private fun onFinish() {
        ...
        if (state is GccMessageSearchViewModel.FinishedState) {
            ...
            .debounce { query ->
                when {
                    TextUtils.isEmpty(query) -> Observable.empty()
                        ...
                        GccConversationsListActivity.SEARCH_DEBOUNCE_INTERVAL_MS.toLong(),
    ...
    companion object {
        const val RESULT_KEY_MESSAGE_ID = "GccMessageSearchActivity.result.message"
        const val RESULT_KEY_THREAD_ID = "GccMessageSearchActivity.result.thread"
    }
}
