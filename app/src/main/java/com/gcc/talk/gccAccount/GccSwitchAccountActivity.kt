...
import com.gcc.talk.gccAdapters.items.GccAdvancedUserItem
import com.gcc.talk.gccApplication.GccTalkApplication
import com.gcc.talk.gccApplication.GccTalkApplication.Companion.sharedApplication
import com.gcc.talk.gccData.user.model.GccUser
import com.gcc.talk.databinding.ActivitySwitchAccountBinding
import com.gcc.talk.gccModels.GccImportAccount
import com.gcc.talk.gccModels.json.participants.Participant
import com.gcc.talk.gccUsers.GccUserManager
import com.gcc.talk.gccUtils.GccAccountUtils.findAvailableAccountsOnDevice
import com.gcc.talk.gccUtils.GccAccountUtils.getInformationFromAccount
...
class GccSwitchAccountActivity : GccBaseActivity() {
    ...
    private val onImportItemClickListener = FlexibleAdapter.OnItemClickListener { _, position ->
        if (userItems.size > position) {
            val account = (userItems[position] as GccAdvancedUserItem).account
    ...
    private val onSwitchItemClickListener = FlexibleAdapter.OnItemClickListener { _, position ->
        if (userItems.size > position) {
            val user = (userItems[position] as GccAdvancedUserItem).user
            ...
            if (!isAccountImport) {
                for (user in userManager.users.blockingGet()) {
                    if (!user.current) {
                        ...
                        userItems.add(GccAdvancedUserItem(participant, user, null, viewThemeUtils, 0))
                    }
                }
                ...
                var account: Account
                var importAccount: GccImportAccount
                var user: GccUser
                for (accountObject in findAvailableAccountsOnDevice(userManager.users.blockingGet())) {
                    ...
                    userItems.add(GccAdvancedUserItem(participant, user, account, viewThemeUtils, 0))
                }
