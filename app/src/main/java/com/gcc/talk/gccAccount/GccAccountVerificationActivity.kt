 ...
 */
package com.gcc.talk.gccAccount
...
import com.gcc.talk.databinding.ActivityAccountVerificationBinding
...
import com.gcc.talk.gccUtils.bundle.GccBundleKeys.KEY_BASE_URL
import com.gcc.talk.gccUtils.bundle.GccBundleKeys.KEY_USERNAME
...
import com.gcc.talk.gccUtils.bundle.GccBundleKeys.KEY_IS_ACCOUNT_IMPORT
import com.gcc.talk.gccUtils.bundle.GccBundleKeys.KEY_ORIGINAL_PROTOCOL
import com.gcc.talk.gccUtils.bundle.GccBundleKeys.KEY_PASSWORD
import com.gcc.talk.gccUtils.bundle.GccBundleKeys.KEY_TOKEN
...
class GccAccountVerificationActivity : GccBaseActivity() {
    ...
    override fun onResume() {
        ...
        if (
            isAccountImport &&
            !GccUriUtils.hasHttpProtocolPrefixed(baseUrl!!) ||
    ...
    private fun determineBaseUrlProtocol(checkForcedHttps: Boolean) {
        ...
        val queryUrl: String = if (checkForcedHttps) {
            "https://" + baseUrl + GccApiUtils.getUrlPostfixForStatus()
        } else {
            "http://" + baseUrl + GccApiUtils.getUrlPostfixForStatus()
        }
    ...
    private fun findServerTalkApp() {
        val credentials = GccApiUtils.getCredentials(username, token)
        cookieManager.cookieStore.removeAll()
            ...
            .subscribe(object : Observer<CapabilitiesOverall> {
                ...
                override fun onNext(capabilitiesOverall: CapabilitiesOverall) {
                    ...
                    if (hasTalk) {
                        ...
                        GccApplicationWideMessageHolder.getInstance().messageType =
    ...
    private fun storeProfile(displayName: String?, userId: String, capabilitiesOverall: CapabilitiesOverall) {
        userManager.storeProfile(
            ...
            .subscribeOn(Schedulers.io())
            .subscribe(object : MaybeObserver<GccUser> {
                override fun onSubscribe(d: Disposable) {
                    disposables.add(d)
                }

                @SuppressLint("SetTextI18n")
                override fun onSuccess(user: GccUser) {
    ...
    private fun fetchProfile(credentials: String, capabilitiesOverall: CapabilitiesOverall) {
        ncApi.getUserProfile(
            credentials,
            GccApiUtils.getUrlForUserProfile(baseUrl!!)
        )
            .subscribeOn(Schedulers.io())
    ...
    fun onMessageEvent(eventStatus: GccEventStatus) {
        Log.d(TAG, "caught GccEventStatus of type " + eventStatus.eventType.toString())
        if (eventStatus.eventType == GccEventStatus.EventType.PUSH_REGISTRATION) {
            ...
            fetchAndStoreCapabilities()
        } else if (eventStatus.eventType == GccEventStatus.EventType.CAPABILITIES_FETCH) {
            if (internalAccountId == eventStatus.userId && !eventStatus.isAllGood) {
                runOnUiThread {
                    binding.progressText.text =
                        """
        ...
        } else if (eventStatus.eventType == GccEventStatus.EventType.SIGNALING_SETTINGS) {
            if (internalAccountId == eventStatus.userId && !eventStatus.isAllGood) {
                runOnUiThread {
                    binding.progressText.text =
                        """
    ...
    private fun fetchAndStoreCapabilities() {
        ...
        val capabilitiesWork =
            OneTimeWorkRequest.Builder(GccCapabilitiesWorker::class.java)
    ...
    private fun fetchAndStoreExternalSignalingSettings() {
        val userData =
            Data.Builder()
                .putLong(KEY_INTERNAL_USER_ID, internalAccountId)
                .build()
        val signalingSettingsWorker = OneTimeWorkRequest.Builder(GccSignalingSettingsWorker::class.java)
            .setInputData(userData)
            .build()
    ...
    private fun proceedWithLogin() {
        ...
        if (userManager.users.blockingGet().size == 1 ||
            ...
            if (userManager.setUserAsActive(userToSetAsActive).blockingGet()) {
                runOnUiThread {
                    if (userManager.users.blockingGet().size == 1) {
                        val intent = Intent(context, GccConversationsListActivity::class.java)
                        startActivity(intent)
                    } else {
                        if (isAccountImport) {
                            GccApplicationWideMessageHolder.getInstance().messageType =
    ...
    private fun abortVerification() {
        if (isAccountImport) {
            GccApplicationWideMessageHolder.getInstance().messageType = GccApplicationWideMessageHolder.MessageType
    ...
    companion object {
        private val TAG = GccAccountVerificationActivity::class.java.simpleName
