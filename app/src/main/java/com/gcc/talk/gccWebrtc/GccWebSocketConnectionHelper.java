...
import com.gcc.talk.gccModels.json.websocket.ActorWebSocketMessage;
import com.gcc.talk.gccModels.json.websocket.AuthParametersWebSocketMessage;
import com.gcc.talk.gccModels.json.websocket.AuthWebSocketMessage;
...
import com.gcc.talk.gccModels.json.websocket.HelloOverallWebSocketMessage;
import com.gcc.talk.gccModels.json.websocket.HelloWebSocketMessage;
import com.gcc.talk.gccModels.json.websocket.RoomFederationWebSocketMessage;
import com.gcc.talk.gccModels.json.websocket.RoomOverallWebSocketMessage;
import com.gcc.talk.gccModels.json.websocket.RoomWebSocketMessage;
...
public class GccWebSocketConnectionHelper {
    public static final String TAG = "GccWebSocketConnectionHelper";
    private static Map<Long, GccWebSocketInstance> webSocketInstanceMap = new HashMap<>();

    @Inject
    OkHttpClient okHttpClient;


    public GccWebSocketConnectionHelper() {
        GccTalkApplication.Companion.getSharedApplication().getComponentApplication().inject(this);
    }

    @SuppressLint("LongLogTag")
    public static synchronized GccWebSocketInstance getWebSocketInstanceForUser(GccUser user) {
        GccWebSocketInstance webSocketInstance = webSocketInstanceMap.get(user.getId());
    ...
    public static synchronized GccWebSocketInstance getExternalSignalingInstanceForServer(String url,
                                                                                          GccUser user,
        ...
        GccWebSocketInstance webSocketInstance = getWebSocketInstanceForUser(user);
        ...
        webSocketInstance = new GccWebSocketInstance(user, generatedURL, webSocketTicket);
    ...
    public static synchronized void deleteExternalSignalingInstanceForUserEntity(long id) {
        GccWebSocketInstance webSocketInstance;
    ...
    HelloOverallWebSocketMessage getAssembledHelloModel(GccUser user, String ticket) {
        ...
        authWebSocketMessage.setUrl(GccApiUtils.getUrlForSignalingBackend(apiVersion, user.getBaseUrl()));
