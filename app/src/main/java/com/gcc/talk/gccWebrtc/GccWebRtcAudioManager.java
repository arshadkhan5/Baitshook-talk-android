...
import com.gcc.talk.gccUtils.GccContextExtensionsKt;
...
public class GccWebRtcAudioManager {
    private static final String TAG = GccWebRtcAudioManager.class.getSimpleName();
    private final Context context;
    private final GccWebRtcBluetoothManager bluetoothManager;
    ...
    private GccProximitySensor proximitySensor = null;
    ...
    private final GccPowerManagerUtils powerManagerUtils;

    private GccWebRtcAudioManager(Context context, boolean useProximitySensor) {
        Log.d(TAG, "ctor");
        ThreadUtils.checkIsOnMainThread();
        this.context = context;
        audioManager = ((AudioManager) context.getSystemService(Context.AUDIO_SERVICE));
        bluetoothManager = GccWebRtcBluetoothManager.create(context, this);
        wiredHeadsetReceiver = new WiredHeadsetReceiver();
        amState = AudioManagerState.UNINITIALIZED;

        powerManagerUtils = new GccPowerManagerUtils();
        ...
        proximitySensor = GccProximitySensor.create(context, new Runnable() {
     ...
     * Construction.
     */
    public static GccWebRtcAudioManager create(Context context, boolean useProximitySensor) {
       return new GccWebRtcAudioManager(context, useProximitySensor);
    }
    ...
    private void onProximitySensorChangedState() {
        ...
        if (userSelectedAudioDevice == AudioDevice.SPEAKER_PHONE
            ...
            if (proximitySensor.sensorReportsNearState()) {
                ...
                EventBus.getDefault().post(new GccProximitySensorEvent(GccProximitySensorEvent.ProximitySensorEventType.SENSOR_NEAR));

            } else {
                setAudioDeviceInternal(GccWebRtcAudioManager.AudioDevice.SPEAKER_PHONE);
                Log.d(TAG, "switched to SPEAKER_PHONE because userSelectedAudioDevice was SPEAKER_PHONE and proximity=far");

                EventBus.getDefault().post(new GccProximitySensorEvent(GccProximitySensorEvent.ProximitySensorEventType.SENSOR_FAR));
            }
        }
    }
    ...
    private void registerReceiver(BroadcastReceiver receiver, IntentFilter filter) {
        GccContextExtensionsKt.registerBroadcastReceiver(context, receiver, filter, GccReceiverFlag.NotExported);
    }
    ...
    public final void updateAudioDeviceState() {
        ...
        if (bluetoothManager.getState() == GccWebRtcBluetoothManager.State.HEADSET_AVAILABLE
            || bluetoothManager.getState() == GccWebRtcBluetoothManager.State.HEADSET_UNAVAILABLE
            || bluetoothManager.getState() == GccWebRtcBluetoothManager.State.SCO_DISCONNECTING) {
            bluetoothManager.updateDevice();
        }

        Set<AudioDevice> newInternalAudioDevices = new HashSet<>();

        if (bluetoothManager.getState() == GccWebRtcBluetoothManager.State.SCO_CONNECTED
            || bluetoothManager.getState() == GccWebRtcBluetoothManager.State.SCO_CONNECTING
            || bluetoothManager.getState() == GccWebRtcBluetoothManager.State.HEADSET_AVAILABLE) {
        ...
        boolean needBluetoothScoStop =
            (bluetoothManager.getState() == GccWebRtcBluetoothManager.State.SCO_CONNECTED
                || bluetoothManager.getState() == GccWebRtcBluetoothManager.State.SCO_CONNECTING)
                && (userSelectedAudioDevice != AudioDevice.NONE
                && userSelectedAudioDevice != AudioDevice.BLUETOOTH);
