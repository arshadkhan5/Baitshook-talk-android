 ...
 */
class GccChunkFromFileRequestBody(
    file: File,
    contentType: MediaType?,
    channel: FileChannel?,
    chunkSize: Long,
    offset: Long,
    listener: GccOnDataTransferProgressListener
) : RequestBody() {
    ...
    private var mDataTransferListener: GccOnDataTransferProgressListener
    ...
    companion object {
        private val TAG = GccChunkFromFileRequestBody::class.java.simpleName
    ...
    init {
        ...
        require(chunkSize > 0) { "GccChunk size must be greater than zero" }
        mFile = file
        mChannel = channel
