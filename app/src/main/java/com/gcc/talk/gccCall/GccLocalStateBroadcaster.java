 ...
 * <p>
 * Once created, and until destroyed, the GccLocalStateBroadcaster will send the changes in the local participant state to
 * all the participants in the call. Note that the GccLocalStateBroadcaster does not check whether the local participant
 * is actually in the call or not; it is expected that the GccLocalStateBroadcaster will be created and destroyed when the
 * local participant joins and leaves the call.
 * <p>
 * The GccLocalStateBroadcaster also sends the current state to remote participants when they join (which implicitly
 ...
 */
public abstract class GccLocalStateBroadcaster {

    private final GccLocalCallParticipantModel localCallParticipantModel;

    private final LocalCallParticipantModelObserver localCallParticipantModelObserver;

    private final GccMessageSender messageSender;

    private class LocalCallParticipantModelObserver implements GccLocalCallParticipantModel.Observer {

        private Boolean audioEnabled;
        private Boolean speaking;
        private Boolean videoEnabled;

        public LocalCallParticipantModelObserver(GccLocalCallParticipantModel localCallParticipantModel) {
    ...
    public GccLocalStateBroadcaster(GccLocalCallParticipantModel localCallParticipantModel,
                                   GccMessageSender messageSender) {
    ...
    public abstract void handleCallParticipantAdded(GccParticipantUiState uiState);
