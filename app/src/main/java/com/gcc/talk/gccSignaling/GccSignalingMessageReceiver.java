...
import com.gcc.talk.gccModels.json.converters.EnumParticipantTypeConverter;
 ...
 * <p>
 * GccSignalingMessageReceiver does not fetch the signaling messages itself; subclasses must fetch them and then call
 * the appropriate protected methods to process the messages and notify the listeners.
 */
public abstract class GccSignalingMessageReceiver {

    private final EnumActorTypeConverter enumActorTypeConverter = new EnumActorTypeConverter();

    private final GccParticipantListMessageNotifier participantListMessageNotifier =
        new GccParticipantListMessageNotifier();

    private final GccLocalParticipantMessageNotifier localParticipantMessageNotifier =
        new GccLocalParticipantMessageNotifier();

    private final GccCallParticipantMessageNotifier callParticipantMessageNotifier =
        new GccCallParticipantMessageNotifier();

    private final GccConversationMessageNotifier conversationMessageNotifier = new GccConversationMessageNotifier();

    private final GccOfferMessageNotifier offerMessageNotifier = new GccOfferMessageNotifier();

    private final GccWebRtcMessageNotifier webRtcMessageNotifier = new GccWebRtcMessageNotifier();
