 ...
 */
package com.gcc.talk.gccConversationlist
...
import com.gcc.talk.gccUi.dialog.ConversationsListBottomDialog
...
import com.gcc.talk.gccAdapters.items.GccContactItem
import com.gcc.talk.gccAdapters.items.GccConversationItem
import com.gcc.talk.gccAdapters.items.GccGenericTextHeaderItem
import com.gcc.talk.gccAdapters.items.GccLoadMoreResultsItem
import com.gcc.talk.gccAdapters.items.GccMessageResultItem
import com.gcc.talk.gccAdapters.items.GccMessagesTextHeaderItem
import com.gcc.talk.gccAdapters.items.GccSpacerItem
...
import com.gcc.talk.gccContacts.ContactsActivity
...
import com.gcc.talk.databinding.ActivityConversationsBinding
import com.gcc.talk.gccAccount.GccBrowserLoginActivity
import com.gcc.talk.gccEvents.GccConversationsListFetchDataEvent
...
import com.gcc.talk.gccJobs.GccContactAddressBookWorker.Companion.run
import com.gcc.talk.gccJobs.GccDeleteConversationWorker
import com.gcc.talk.gccJobs.GccUploadAndShareFilesWorker
import com.gcc.talk.gccMessagesearch.GccMessageSearchHelper
import com.gcc.talk.gccMessagesearch.GccMessageSearchHelper.MessageSearchResults
...
import com.gcc.talk.gccSettings.GccSettingsActivity
import com.gcc.talk.gccThreadsoverview.ThreadsOverviewActivity
import com.gcc.talk.gccUi.BackgroundVoiceMessageCard
import com.gcc.talk.gccUi.dialog.ChooseAccountDialogCompose
import com.gcc.talk.gccUi.dialog.ChooseAccountShareToDialogFragment
import com.gcc.talk.gccUi.dialog.FilterConversationFragment
import com.gcc.talk.gccUi.dialog.FilterConversationFragment.Companion.ARCHIVE
import com.gcc.talk.gccUi.dialog.FilterConversationFragment.Companion.MENTION
import com.gcc.talk.gccUi.dialog.FilterConversationFragment.Companion.UNREAD
...
import com.gcc.talk.gccUtils.CapabilitiesUtil.isServerEOL
...
import com.gcc.talk.gccUtils.bundle.GccBundleKeys.ADD_ADDITIONAL_ACCOUNT
import com.gcc.talk.gccUtils.bundle.GccBundleKeys.KEY_FORWARD_HIDE_SOURCE_ROOM
import com.gcc.talk.gccUtils.bundle.GccBundleKeys.KEY_FORWARD_MSG_FLAG
import com.gcc.talk.gccUtils.bundle.GccBundleKeys.KEY_FORWARD_MSG_TEXT
...
import com.gcc.talk.gccUtils.bundle.GccBundleKeys.KEY_SCROLL_TO_NOTIFICATION_CATEGORY
import com.gcc.talk.gccUtils.bundle.GccBundleKeys.KEY_SHARED_TEXT
...
import com.gcc.talk.gccUtils.rx.GccSearchViewObservable.Companion.observeSearchView
...
class GccConversationsListActivity :
    GccBaseActivity(),
    ...
    @Inject
    lateinit var unifiedSearchRepository: GccUnifiedSearchRepository
    ...
    @Inject
    lateinit var contactsViewModel: GccContactsViewModel

    lateinit var conversationsListViewModel: GccConversationsListViewModel
    lateinit var contextChatViewModel: GccContextChatViewModel

    override val appBarLayoutType: AppBarLayoutType
        get() = AppBarLayoutType.SEARCH_BAR

    private var currentUser: GccUser? = null
    ...
    private var selectedConversation: GccConversationModel? = null
    ...
    private val callHeaderItems = HashMap<String, GccGenericTextHeaderItem>()
    private var conversationsListBottomDialog: ConversationsListBottomDialog? = null
    private var searchHelper: GccMessageSearchHelper? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        conversationsListViewModel = ViewModelProvider(this, viewModelFactory)[GccConversationsListViewModel::class.java]
        ...
        if (currentUser != null) {
            ...
            currentUser?.capabilities?.spreedCapability?.let { spreedCapabilities ->
                if (hasSpreedFeatureCapability(spreedCapabilities, SpreedFeatures.UNIFIED_SEARCH)) {
                    searchHelper = GccMessageSearchHelper(
    ...
    override fun onPause() {
        ...
        val firstTop = (firstItem as? GccConversationItem)?.mHolder?.itemView?.top
    ...
    private fun addEmptyItemForEdgeToEdgeIfNecessary() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.VANILLA_ICE_CREAM) {
            adapter?.addScrollableFooter(GccSpacerItem(100))
        }
    }
    ...
    private fun initObservers() {
        ...
        conversationsListViewModel.getFederationInvitationsViewState.observe(this) { state ->
            when (state) {
                is GccConversationsListViewModel.GetFederationInvitationsStartState -> {
                    binding.conversationListHintInclude.conversationListHintLayout.visibility = View.GONE
                }

                is GccConversationsListViewModel.GetFederationInvitationsSuccessState -> {
                ...
                is GccConversationsListViewModel.GetFederationInvitationsErrorState -> {
                    // do nothing
                }
        ...
        conversationsListViewModel.showBadgeViewState.observe(this) { state ->
            when (state) {
                is GccConversationsListViewModel.ShowBadgeStartState -> {
                    showAccountIconBadge(false)
                }

                is GccConversationsListViewModel.ShowBadgeSuccessState -> {
                    showAccountIconBadge(state.showBadge)
                }

                is GccConversationsListViewModel.ShowBadgeErrorState -> {
                    Snackbar.make(binding.root, R.string.nc_common_error_sorry, Snackbar.LENGTH_LONG).show()
                }
        ...
        conversationsListViewModel.getRoomsViewState.observe(this) { state ->
            when (state) {
                is GccConversationsListViewModel.GetRoomsSuccessState -> {
                    if (adapterWasNull) {
                ...
                is GccConversationsListViewModel.GetRoomsErrorState -> {
        ...
        lifecycleScope.launch {
            conversationsListViewModel.threadsExistState.collect { state ->
                when (state) {
                    is GccConversationsListViewModel.ThreadsExistUiState.Success -> {
        ...
        lifecycleScope.launch {
            conversationsListViewModel.openConversationsState.collect { state ->
                when (state) {
                    is GccConversationsListViewModel.OpenConversationsUiState.Success -> {
                        ...
                        for (conversation in state.conversations) {
                            var genericTextHeaderItem: GccGenericTextHeaderItem
                            if (!callHeaderItems.containsKey(headerTitle)) {
                                genericTextHeaderItem = GccGenericTextHeaderItem(headerTitle, viewThemeUtils)
                                callHeaderItems[headerTitle] = genericTextHeaderItem
                            }
                            val conversationItem = GccConversationItem(
                                GccConversationModel.mapToConversationModel(conversation, currentUser!!),
                                currentUser!!,
                                this@GccConversationsListActivity,
                        ...
                        mutex.withLock {
                            ...
                            searchableConversationItems = searchableConversationItems.filter {
                                !(it is GccConversationItem && it.header == callHeaderItems[headerTitle])
                            }.toMutableList()

                            searchableConversationItems.addAll(openConversationItems)
                        }
                    }
                    is GccConversationsListViewModel.OpenConversationsUiState.Error -> {
        ...
        lifecycleScope.launch {
            conversationsListViewModel.getRoomsFlow
                .onEach { list ->
                    ...
                    val noteToSelf = list
                        .firstOrNull { GccConversationUtils.isNoteToSelfConversation(it) }
        ...
        lifecycleScope.launch {
            contactsViewModel.roomViewState.onEach { state ->
                when (state) {
                    is GccContactsViewModel.RoomUiState.Success -> {
                ...
                when (state) {
                    is GccContactsViewModel.ContactsUiState.Success -> {
                        ...
                        for (autocompleteUser in state.contacts) {
                            ...
                            val contactItem = GccContactItem(
                                participant,
                                currentUser!!,
                                callHeaderItems[headerTitle],
                                viewThemeUtils
                ...
                binding.composeViewForBackgroundPlay.apply {
                    ...
                    setContent {
                        msg?.let {
                            ...
                            val imageURI = GccApiUtils.getUrlForAvatar(
                                currentUser?.baseUrl,
                                msg.actorId,
                                true
                            )
                            val conversationImageURI = GccApiUtils.getUrlForConversationAvatar(
                                GccApiUtils.API_V1,
                                currentUser?.baseUrl,
                                msg.token
                            )
    ...
    private fun handleNoteToSelfShortcut(noteToSelfAvailable: Boolean, noteToSelfToken: String) {
        if (noteToSelfAvailable) {
            ...
            bundle.putBoolean(GccBundleKeys.KEY_FOCUS_INPUT, true)
    ...
    private fun setConversationList(list: List<GccConversationModel>) {
    ...
    private fun isFutureEvent(conversation: GccConversationModel): Boolean {
    ...
    private fun addToNearFutureEventConversationItems(conversation: GccConversationModel) {
        val conversationItem = GccConversationItem(conversation, currentUser!!, this, null, viewThemeUtils)
    ...
    fun filterConversation() {
        ...
        for (i in items) {
            val conversation = (i as GccConversationItem).model
    ...
    private fun filter(conversation: GccConversationModel): Boolean {
        var result = true
        for ((k, v) in filterState) {
            if (v) {
                when (k) {
    ...
    private fun loadUserAvatar(target: Target) {
        if (currentUser != null) {
            val url = GccApiUtils.getUrlForAvatar(
                currentUser!!.baseUrl!!,
                currentUser!!.userId,
                true
            )
    ...
    private fun showChooseAccountDialog() {
        binding.genericComposeView.apply {
            ...
            setContent {
                ChooseAccountDialogCompose().GetChooseAccountDialog(shouldDismiss, this@GccConversationsListActivity)
            }
        }
    }
        ...
        if (showShareToScreen) {
            ...
            searchView!!.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
                ...
                override fun onQueryTextChange(p0: String?): Boolean {
                    this@GccConversationsListActivity.onQueryTextChange(p0)
                    return true
                }
            })
            ...
            searchItem!!.setOnActionExpandListener(object : MenuItem.OnActionExpandListener {
                ...
                override fun onMenuItemActionCollapse(item: MenuItem): Boolean {
                    ...
                    if (resources != null) {
                        viewThemeUtils.platform.resetStatusBar(this@GccConversationsListActivity)
                    }
    ...
    private fun fetchPendingInvitations() {
        if (hasSpreedFeatureCapability(currentUser?.capabilities?.spreedCapability, SpreedFeatures.FEDERATION_V1)) {
            binding.conversationListHintInclude.conversationListHintLayout.setOnClickListener {
                val intent = Intent(this, GccInvitationsActivity::class.java)
    ...
    private fun addToConversationItems(conversation: GccConversationModel) {
        ...
        val genericTextHeaderItem: GccGenericTextHeaderItem
        if (!callHeaderItems.containsKey(headerTitle)) {
            genericTextHeaderItem = GccGenericTextHeaderItem(headerTitle, viewThemeUtils)
            callHeaderItems[headerTitle] = genericTextHeaderItem
        }
        ...
        val conversationItemWithHeader = GccConversationItem(
            conversation,
            currentUser!!,
            this,
            callHeaderItems[headerTitle],
        ...
        binding.floatingActionButton.let {
            ...
            if (resources!!.getBoolean(R.bool.multiaccount_support)) {
                dialogBuilder.setNeutralButton(R.string.nc_account_chooser_add_account) { _, _ ->
                    val intent = Intent(this, GccServerSelectionActivity::class.java)
    ...
    private fun sortConversations(conversationItems: MutableList<AbstractFlexibleItem<*>>) {
        conversationItems.sortWith { o1: AbstractFlexibleItem<*>, o2: AbstractFlexibleItem<*> ->
            val conversation1 = (o1 as GccConversationItem).model
            val conversation2 = (o2 as GccConversationItem).model
            CompareToBuilder()
    ...
    private fun prepareViews() {
        ...
        binding.switchAccountButton.setOnClickListener {
            if (resources != null && resources!!.getBoolean(R.bool.multiaccount_support)) {
                ...
                val intent = Intent(context, GccSettingsActivity::class.java)
                startActivity(intent)
            }
        }
            ...
            if (value) {
                ...
                try {
                    ...
                    for (flexItem in conversationItems) {
                        val conversation: GccConversationModel = (flexItem as GccConversationItem).model
    ...
    private fun hasUnreadItems(conversation: GccConversationModel) =
                ...
                mutex.withLock {
                    if (hasFilterEnabled()) {
                        ...
                        fun AbstractFlexibleItem<*>.isRegularConversationItem() =
                            this is GccConversationItem && this.header != callHeaderItems[headerTitle]
    ...
    private fun clearMessageSearchResults() {
        ...
        if (firstHeader != null && firstHeader.itemViewType == GccMessagesTextHeaderItem.VIEW_TYPE) {
            adapter?.removeSection(firstHeader)
        } else {
            adapter?.removeItemsOfType(GccMessageResultItem.VIEW_TYPE)
            adapter?.removeItemsOfType(GccMessagesTextHeaderItem.VIEW_TYPE)
        }
        adapter?.removeItemsOfType(GccLoadMoreResultsItem.VIEW_TYPE)
    }
    ...
    override fun onItemClick(view: View, position: Int): Boolean {
        ...
        if (item != null) {
            when (item) {
                is GccMessageResultItem -> {
                    val token = item.messageEntry.conversationToken
                    (
                        conversationItems.first {
                            (it is GccConversationItem) && it.model.token == token
                        } as GccConversationItem
                        ).model.displayName
                ...
                is GccLoadMoreResultsItem -> {
                    loadMoreMessages()
                }

                is GccConversationItem -> {
    ...
    private fun showConversationByToken(conversationToken: String) {
        for (absItem in conversationItems) {
            val conversationItem = absItem as GccConversationItem
    ...
    private fun handleConversation(conversation: GccConversationModel?) {
        selectedConversation = conversation
        if (selectedConversation != null) {
            val hasChatPermission = GccParticipantPermissions(
                currentUser?.capabilities?.spreedCapability,
    ...
    private fun shouldShowLobby(conversation: GccConversationModel): Boolean {
        val participantPermissions = GccParticipantPermissions(
            currentUser?.capabilities?.spreedCapability,
            selectedConversation!!
        )
        return conversation.lobbyState == ConversationEnums.LobbyState.LOBBY_STATE_MODERATORS_ONLY &&
            !GccConversationUtils.canModerate(conversation, currentUser?.capabilities?.spreedCapability) &&
            !participantPermissions.canIgnoreLobby()
    }

    private fun isReadOnlyConversation(conversation: GccConversationModel): Boolean =
    ...
    override fun onItemLongClick(position: Int) {
        this.lifecycleScope.launch {
            if (showShareToScreen || !networkMonitor.isOnline.value) {
                ...
                if (clickedItem != null && clickedItem is GccConversationItem) {
        ...
        if (intent != null) {
            if (Intent.ACTION_SEND == intent.action || Intent.ACTION_SEND_MULTIPLE == intent.action) {
                try {
                    ...
                    if (GccMimetype.TEXT_PLAIN == mimeType && intent.getStringExtra(Intent.EXTRA_TEXT) != null) {
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<String>, grantResults: IntArray) {
        ...
        when (requestCode) {
            GccUploadAndShareFilesWorker.REQUEST_PERMISSION -> {
            ...
            REQUEST_POST_NOTIFICATIONS_PERMISSION -> {
                if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    if (!GccPowerManagerUtils().isIgnoringBatteryOptimizations() &&
                        ClosedInterfaceImpl().isGooglePlayServicesAvailable
                    ) {
    ...
    private fun showNotificationWarning() {
        if (shouldShowNotificationWarning()) {
            ...
            binding.conversationListNotificationWarning.showSettingsButton.setOnClickListener {
                ...
                val settingsIntent = Intent(context, GccSettingsActivity::class.java)
    ...
    private fun shouldShowNotificationWarning(): Boolean {
        ...
        val notificationPermissionNotGranted = Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU &&
            !platformPermissionUtil.isPostNotificationsPermissionGranted()
        val batteryOptimizationNotIgnored = !GccPowerManagerUtils().isIgnoringBatteryOptimizations()

        val messagesChannelNotEnabled = !GccNotificationUtils.isMessagesNotificationChannelEnabled(this)
        val callsChannelNotEnabled = !GccNotificationUtils.isCallsNotificationChannelEnabled(this)
    ...
    private fun openConversation(textToPaste: String? = "") {
        if (GccCallActivity.active &&
            selectedConversation!!.token != GccApplicationWideCurrentRoomHolder.getInstance().currentRoomToken
        ) {
            Snackbar.make(
                binding.root,
        ...
        if (selectedMessageId != null) {
            bundle.putString(GccBundleKeys.KEY_MESSAGE_ID, selectedMessageId)
    ...
    fun onMessageEvent(eventStatus: GccEventStatus) {
        if (currentUser != null && eventStatus.userId == currentUser!!.id) {
            when (eventStatus.eventType) {
                GccEventStatus.EventType.CONVERSATION_UPDATE -> if (eventStatus.isAllGood && !isRefreshing) {
                    fetchRooms()
                }
    ...
    fun onMessageEvent(conversationsListFetchDataEvent: GccConversationsListFetchDataEvent?) {
        fetchRooms()
        Handler().postDelayed({
    ...
    fun showDeleteConversationDialog(conversation: GccConversationModel) {
        binding.floatingActionButton.let {
            val dialogBuilder = MaterialAlertDialogBuilder(it.context)
                .setIcon(
                    viewThemeUtils.dialog
        ...
        binding.floatingActionButton.let {
            val dialogBuilder = MaterialAlertDialogBuilder(it.context)
                ...
                .setNegativeButton(R.string.nc_settings_reauthorize) { _, _ ->
                    ...
                    val bundle = Bundle()
                    bundle.putString(GccBundleKeys.KEY_BASE_URL, currentUser!!.baseUrl!!)
                    bundle.putBoolean(GccBundleKeys.KEY_REAUTHORIZE_ACCOUNT, true)
                    intent.putExtras(bundle)
                    startActivity(intent)
                }
    ...
    private fun onMessageSearchResult(results: MessageSearchResults) {
        if (searchView!!.query.isNotEmpty()) {
            ...
            if (entries.isNotEmpty()) {
                ...
                if (results.hasMore) {
                    adapterItems.add(GccLoadMoreResultsItem)
                }
    ...
    companion object {
        private val TAG = GccConversationsListActivity::class.java.simpleName
        const val UNREAD_BUBBLE_DELAY = 2500
        const val BOTTOM_SHEET_DELAY: Long = 2500
        private const val KEY_SEARCH_QUERY = "GccConversationsListActivity.searchQuery"
        private const val CHAT_ACTIVITY_LOCAL_NAME = "com.nextcloud.talk.chat.GccChatActivity"
