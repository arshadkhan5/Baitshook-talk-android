 ...
 */
package com.gcc.talk.gccTranslate.viewmodels
...
class GccTranslateViewModel @Inject constructor(
    private val repository: GccTranslateRepository,
    ...
    class LanguagesRetrievedState(val list: List<GccLanguage>) : ViewState
    ...
    fun translateMessage(toLanguage: String, fromLanguage: String?, text: String) {
        val currentUser: GccUser = currentUserProvider.currentUser.blockingGet()
        val authorization: String = GccApiUtils.getCredentials(currentUser.username, currentUser.token)!!
        val url: String = GccApiUtils.getUrlForTranslation(currentUser.baseUrl!!)
    ...
    fun getLanguages() {
        val currentUser: GccUser = currentUserProvider.currentUser.blockingGet()
        val authorization: String = GccApiUtils.getCredentials(currentUser.username, currentUser.token)!!
        val url: String = GccApiUtils.getUrlForLanguages(currentUser.baseUrl!!)
        Log.d(TAG, "URL is: $url")
        repository.getLanguages(authorization, url)
            .subscribeOn(Schedulers.io())
            ?.observeOn(AndroidSchedulers.mainThread())
            ?.subscribe(object : Observer<List<GccLanguage>> {
                ...
                override fun onNext(list: List<GccLanguage>) {
    ...
    companion object {
        private val TAG = GccTranslateViewModel::class.simpleName
    }
}
