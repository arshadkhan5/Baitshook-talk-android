 ...
 */
package com.gcc.talk.gccUtils.preferences
...
class GccAppPreferencesImpl (val context: Context) : GccAppPreferences {

    override fun getProxyType(): String =
        runBlocking {
            async { readString(PROXY_TYPE, context.resources.getString(string.nc_no_proxy)).first() }
        }.getCompleted()
    ...
    override fun getScreenLockTimeout(): String {
        val default = context.resources.getString(string.nc_screen_lock_timeout_sixty)
    ...
    override fun getTheme(): String {
        val key = context.resources.getString(string.nc_settings_theme_key)
        val default = context.resources.getString(string.nc_default_theme)
    ...
    override fun setReadPrivacy(value: Boolean) =
        runBlocking<Unit> {
            val key = context.resources.getString(string.nc_settings_read_privacy_key)
            async {
    ...
    override fun setSorting(value: String?) =
        runBlocking<Unit> {
            val key = context.resources.getString(string.nc_file_browser_sort_by_key)
            async {
                if (value != null) {
                    writeString(key, value)
                }
    ...
    override fun getSorting(): String {
        val key = context.resources.getString(string.nc_file_browser_sort_by_key)
        val default = context.resources.getString(string.nc_file_browser_sort_by_default)
        val read = runBlocking { async { readString(key).first() } }.getCompleted()
        return read.ifEmpty { default }
    }
    ...
    companion object {
        ...
        private val TAG = GccAppPreferencesImpl::class.simpleName
