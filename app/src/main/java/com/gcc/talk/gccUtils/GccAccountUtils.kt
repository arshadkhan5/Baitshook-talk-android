...
object GccAccountUtils {

    private const val TAG = "GccAccountUtils"
    private const val MIN_SUPPORTED_FILES_APP_VERSION = 30060151

    fun findAvailableAccountsOnDevice(users: List<GccUser>): List<Account> {
        val context = GccTalkApplication.sharedApplication!!.applicationContext
    ...
    private fun matchAccounts(importAccount: GccImportAccount, user: GccUser): Boolean {
        var accountFound = false
        if (importAccount.token != null) {
            if (GccUriUtils.hasHttpProtocolPrefixed(importAccount.baseUrl!!)) {
                if (
    ...
    fun getInformationFromAccount(account: Account): GccImportAccount {
        ...
        return GccImportAccount(username, password, urlString)
    }
}
