...
object CapabilitiesUtil {
    ...
    @JvmStatic
    fun isLinkPreviewAvailable(user: GccUser): Boolean =
        user.capabilities?.coreCapability?.referenceApi != null &&
            user.capabilities?.coreCapability?.referenceApi == "true"

    fun canGeneratePrettyURL(user: GccUser): Boolean = user.capabilities?.coreCapability?.modRewriteWorking == true
    ...
    fun isCallReactionsSupported(user: GccUser?): Boolean {
    ...
    fun isReadStatusPrivate(user: GccUser): Boolean {
    ...
    fun isTypingStatusAvailable(user: GccUser): Boolean {
        if (user.capabilities?.spreedCapability?.config?.containsKey("chat") == true) {
            val map = user.capabilities!!.spreedCapability!!.config!!["chat"]
    ...
    fun isTypingStatusPrivate(user: GccUser): Boolean {
        if (user.capabilities?.spreedCapability?.config?.containsKey("chat") == true) {
            val map = user.capabilities!!.spreedCapability!!.config!!["chat"]
    ...
    fun isFederationAvailable(user: GccUser): Boolean =
    ...
    fun getServerName(user: GccUser?): String? {
    ...
    fun canEditScopes(user: GccUser): Boolean =
    ...
    @JvmStatic
    fun isUserStatusAvailable(user: GccUser): Boolean =
        user.capabilities?.userStatusCapability?.enabled == true &&
            user.capabilities?.userStatusCapability?.supportsEmoji == true

    fun isRestoreStatusAvailable(user: GccUser): Boolean = user.capabilities?.userStatusCapability?.restore == true
