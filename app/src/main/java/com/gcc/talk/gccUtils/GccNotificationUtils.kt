...
object GccNotificationUtils {
    ...
    private fun createCallsNotificationChannel(context: Context, appPreferences: GccAppPreferences) {
        val audioAttributes =
    ...
    private fun createMessagesNotificationChannel(context: Context, appPreferences: GccAppPreferences) {
        val audioAttributes =
            AudioAttributes.Builder()
                .setContentType(AudioAttributes.CONTENT_TYPE_SONIFICATION)
    ...
    fun registerNotificationChannels(context: Context, appPreferences: GccAppPreferences) {
    ...
    private inline fun scanNotifications(
        ...
        for (statusBarNotification in statusBarNotifications) {
            ...
            if (
                ...
                conversationUser.id == notification.extras.getLong(GccBundleKeys.KEY_INTERNAL_USER_ID)
            ) {
    ...
    fun cancelAllNotificationsForAccount(context: Context?, conversationUser: GccUser) {
        scanNotifications(context, conversationUser) { notificationManager, statusBarNotification, _ ->
            notificationManager.cancel(statusBarNotification.id)
        }
    }

    fun cancelNotification(context: Context?, conversationUser: GccUser, notificationId: Long?) {
        scanNotifications(context, conversationUser) { notificationManager, statusBarNotification, notification ->
            if (notificationId == notification.extras.getLong(GccBundleKeys.KEY_NOTIFICATION_ID)) {
                notificationManager.cancel(statusBarNotification.id)
            }
        }
    }
    ...
    fun findNotificationForRoom(
        ...
        scanNotifications(context, conversationUser) { _, statusBarNotification, notification ->
            if (roomTokenOrId == notification.extras.getString(GccBundleKeys.KEY_ROOM_TOKEN)) {
    ...
    fun cancelExistingNotificationsForRoom(context: Context?, conversationUser: GccUser, roomTokenOrId: String) {
        scanNotifications(context, conversationUser) { notificationManager, statusBarNotification, notification ->
            if (roomTokenOrId == notification.extras.getString(GccBundleKeys.KEY_ROOM_TOKEN) &&
                !notification.extras.getBoolean(GccBundleKeys.KEY_NOTIFICATION_RESTRICT_DELETION)
            ) {
                notificationManager.cancel(statusBarNotification.id)
            }
        }
        ...
        return if (TextUtils.isEmpty(ringtonePreferencesString)) {
            ...
            try {
                val ringtoneSettings =
                    LoganSquare.parse(ringtonePreferencesString, GccRingtoneSettings::class.java)
    ...
    fun getCallRingtoneUri(context: Context, appPreferences: GccAppPreferences): Uri? =
        getRingtoneUri(
            context,
    ...
    fun getMessageRingtoneUri(context: Context, appPreferences: GccAppPreferences): Uri? =
        getRingtoneUri(
            context,
